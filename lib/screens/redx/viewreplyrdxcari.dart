...
import 'package:google_map_live/core/models/listviewreplyredx.dart';
...
class Allreplyredx extends StatefulWidget {
  final String idredx;

  const Allreplyredx({Key key, this.idredx}) : super(key: key);

  @override
  _AllreplyredxState createState() => _AllreplyredxState();
}

class _AllreplyredxState extends State<Allreplyredx> {
  //Theme Data
  ...
  final listvidio = new List<Listviewreplyredx>();

  var loadingdokter = false;

  List<Listviewreplyredx> _list = [];
  List<Listviewreplyredx> _search = [];
  ...
  getCurrentLocation() async {
    ...
    final responsee = await http.post(Uri.parse(RestApi.hasilreplyredx),
        body: {"idredx": widget.idredx});
    ...
    if (responsee.statusCode == 200) {
      ...
      setState(() {
        for (Map i in datadokter) {
          _list.add(Listviewreplyredx.fromJson(i));
          loadingdokter = false;
        }
      });
    } else {
