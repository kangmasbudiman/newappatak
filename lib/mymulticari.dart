...
import 'package:google_map_live/core/models/listmymulitiku.dart';
...
import 'package:google_map_live/detailmymulti.dart';
...
class Mymultipointcari extends StatefulWidget {
  @override
  _MymultipointcariState createState() => _MymultipointcariState();
}

class _MymultipointcariState extends State<Mymultipointcari> {
  //Theme Data
  ...
  final listvidio = new List<Listmymulticycari>();

  var loadingdokter = false;

  List<Listmymulticycari> _list = [];
  List<Listmymulticycari> _search = [];
  ...
  getCurrentLocation() async {
    ...
    final responsee = await http.post(Uri.parse(RestApi.getmymulti), body: {
      "idgroup": idgroup1,
    });
    ...
    if (responsee.statusCode == 200) {
      ...
      setState(() {
        for (Map i in datadokter) {
          _list.add(Listmymulticycari.fromJson(i));
          loadingdokter = false;
        }
      });
    } else {
  ...
  _onsearch(String text) async {
    ...
    _list.forEach((f) {
      if (f.name.contains(text) || f.name.contains(text)) _search.add(f);
    });
    setState(() {});
  }
  ...
  _hapus(int id) async {
    final response = await http.post(Uri.parse(RestApi.deletemymulti), body: {
      "id": id.toString(),
    });
