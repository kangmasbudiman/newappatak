...
import 'package:google_map_live/core/models/listgoto.dart';
...
class Allgotolist extends StatefulWidget {
  @override
  _AllgotolistState createState() => _AllgotolistState();
}

class _AllgotolistState extends State<Allgotolist> {
  //Theme Data
  ...
  final listvidio = new List<Listgoto>();

  var loadingdokter = false;

  List<Listgoto> _list = [];
  List<Listgoto> _search = [];
  ...
  getCurrentLocation() async {
    ...
    final response = await http.post(Uri.parse(RestApi.getgoto), body: {
      "idgroup": idgroup1,
    });
  ...
  deleteimagelokasi(String id) async {
    final response = await http.post(Uri.parse(RestApi.deletebygoto), body: {
      "id": id,
    });
